			...
			<div class="row">
				<div class="col-lg-12 col-md-12 sm-padding-l p-r user-p-exist">
					<ul class="page-tabs nav nav-tabs" role="tablist">
						...
						<li><a class="active" href="{site_url_multi('user/discount')}">{translate('discount')}</a></li>
					</ul>
					<div class="combo-jumbotron comment-form">
						<div class="comment-form-header">
							<h5>{translate('all_discount')}</h5>
							<a href="{site_url_multi('discount/add')}"><i class="fas fa-plus"></i> {translate('add_discount')}</a>
						</div>
							...
							<table>
								<thead>
									<tr>
										...
										<th>{translate('discount_company')}</th>
										<th>{translate('discount_status')}</th>
										<th>{translate('discount_date')}</th>
										<th>{translate('discount_action')}</th>
									</tr>
								</thead>
								<tbody>
									...
									{else}
									<tr>
										<td colspan="5"><div class="alert alert-danger">{translate('discount_not_found')}</div></td>
									</tr>
									{/if}
								</tbody>
							</table>
