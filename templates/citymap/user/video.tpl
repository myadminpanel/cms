			...
			<div class="row">
				<div class="col-lg-12 col-md-12 sm-padding-l p-r user-p-exist">
					<ul class="page-tabs nav nav-tabs" role="tablist">
						...
						<li><a class="active" href="{site_url_multi('user/video')}">{translate('video')}</a></li>
					...
					<div class="combo-jumbotron comment-form">
						<div class="comment-form-header">
							<h5>{translate('all_video')}</h5>
							<a href="{site_url_multi('video/add')}"><i class="fas fa-plus"></i> {translate('add_video')}</a>
						</div>
						<div class="style-table">
							<table>
								<thead>
									<tr>
										<th>{translate('video_name')}</th>
										<th>{translate('video_company')}</th>
										<th>{translate('video_status')}</th>
										<th>{translate('video_date')}</th>
										<th>{translate('video_action')}</th>
									</tr>
								</thead>
								<tbody>
									...
									{else}
									<tr>
										<td colspan="5"><div class="alert alert-danger">{translate('video_not_found')}</div></td>
									</tr>
									{/if}
								</tbody>
							</table>
