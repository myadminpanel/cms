			...
			<div class="row">
				<div class="col-lg-12 col-md-12 sm-padding-l p-r user-p-exist">
					<ul class="page-tabs nav nav-tabs" role="tablist">
						...
						<li><a class="active" href="{site_url_multi('user/news')}">{translate('news')}</a></li>
					...
					<div class="combo-jumbotron comment-form">
						<div class="comment-form-header">
							<h5>{translate('all_news')}</h5>
							<a href="{site_url_multi('news/add')}"><i class="fas fa-plus"></i> {translate('add_news')}</a>
						</div>
						<div class="style-table">
							<table>
								<thead>
									<tr>
										<th>{translate('news_name')}</th>
										<th>{translate('news_company')}</th>
										<th>{translate('news_status')}</th>
										<th>{translate('news_date')}</th>
										<th>{translate('news_action')}</th>
									</tr>
								</thead>
								<tbody>
									...
									{else}
									<tr>
										<td colspan="5"><div class="alert alert-danger">{translate('news_not_found')}</div></td>
									</tr>
									{/if}
								</tbody>
							</table>
