							...
							<form method="POST" action="{current_url()}">
								<div class="comment-form">
										...
										<select name="company_id" class="form-control">
											...
											{if $companies}
												{foreach from=$companies item=company}
													<option {if $video->company_id eq $company->id}selected="selected"{/if} value="{$company->id}">{$company->name}</option>
												{/foreach}
											{/if}
										</select>
									</div>
									<div class="form-group">
										...
										<input type="text" name="video" class="form-control" value="{if $video->video}{$video->video}{else}{set_value('video')}{/if}" placeholder="{translate('youtube_url')}" />
									</div>
									<div class="form-group">
