...
<!-- ==== ==== ==== ==== User Profile start ==== ==== ==== ==== -->
					...
					<div class="combo-jumbotron tab-content">
						<div class="comment-form">
							<div class="comment-form-header">
								<h3> {$title} </h3>
								{if count($companies) > 0}<a href="#" class="favorite_delete_all"> <i class="fas fa-trash-alt"></i> {translate('remove_favorite_all')} </a>{/if}
							</div>
									...
									<ul class="last-added-ul" >
										...
										<li>
											...
											<div class="last-added-content">
												...
												<button type="button" data-id="{$row->id}" class="btn btn-sm btn-block btn-danger favorite_delete"><i class="fas fa-minus-circle"></i> {translate('remove_favorite')}</button>
											</div>
										</li>
										{/foreach}
									</ul>
	...
	<script type="text/javascript">
	$('.favorite_delete_all').on('click', function(){
		let element = $(this);
		let company_id = $(this).data('id');
		$.ajax({
			type: "POST",
			url: '/favorite/remove_all',
			dataType: 'json',
			success: function (response) {
				if(response.success)
				{
