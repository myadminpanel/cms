		...
		<table class="table table-bordered table-striped table-hover table-xxs">
			...
			<tbody>
				...
				{if isset($files) && !empty($files)}
					...
					<tr>
						<td><i class="icon-file-css2"></i> <a href="{site_url($admin_url)}/translation/file/{$file.path}">{$file.name}</td>
						<td>
							<ul class="icons-list">
								<li><a href="{site_url($admin_url)}/translation/file/{$file.href}" data-popup="tooltip" title="{translate('edit', true)}"><i class="icon-pencil7"></i></a></li>
							</ul>
						</td>
					</tr>
					{/foreach}
