...
<div class="panel panel-white">
	<div class="panel-heading">
		...
		<div class="heading-elements">
			<div class="btn-group">
				{if isset($language_list_holder) && is_array($language_list_holder)}
					{foreach $language_list_holder as $language}		
						<a href="{$language.link}" class="{$language.class}">
							<img src="{$admin_theme}/global_assets/images/flags/{$language.code}.png" alt="{$language.name}"> {$language.name|upper} <span class="label label-success">{$language.count}</span><span class="label bg-danger">{$language.uncount}</span></a>
					{/foreach}
				{/if}
			</div>
			...
			{if isset($search_field)}
				{form_open(current_full_url(), 'class="heading-form pull-right" method="GET"')}
					<div class="form-group has-feedback">
						{form_element($search_field)}
	...
	{if isset($message) && !empty($message)}
		<div class="panel-body">
			<div class="alert alert-{$message.type} no-border">
				{$message.text}
			</div>
		</div>
	{/if}
	...
	<div class="panel-footer">
		...
		<div class="heading-elements">
			<span class="heading-left-element" style="padding-left:10px;">
				{form_dropdown('per_page', $per_page_lists, $limit.per_page, ["class" => "bootstrap-select", "data-style" => "btn-default btn-xs"])}
			</span>
			{$pagination}
		</div>
	</div>
