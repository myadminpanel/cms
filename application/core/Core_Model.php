...
class Core_Model extends CI_Model
{
	...
	*/
	...
	public function without_translation($language_id = false, $default_language_id)
	{

		/*
			AND NOT EXISTS (
			SELECT `cm_company_translation`.`company_id` FROM `cm_company_translation`
			WHERE `cm_company_translation`.`language_id` = 4 AND `cm_company_translation`.`company_id`=`cm_company`.`id`
			) 
		*/
		if ($language_id)
		{
			$this->where(['NOT EXISTS (
				SELECT `'.$this->db->dbprefix($this->table_translation).'`.`'.$this->table_translation_key.'` FROM `'.$this->db->dbprefix($this->table_translation).'`
				WHERE `'.$this->db->dbprefix($this->table_translation).'`.`language_id` = '.$language_id.' AND `'.$this->db->dbprefix($this->table_translation).'`.`'.$this->table_translation_key.'`=`'.$this->table.'`.`id`)' => NULL]);
		}
		$this->where([$this->table_language_key => $default_language_id]);
		$this->database->join($this->table_translation, $this->table_translation.'.'.$this->table_translation_key.' = '.$this->table.'.'.$this->primary_key, 'left');
	...
	public function update_translation($data, $where = [])
	{
		$this->database->where($where);
		$this->database->update($this->table_translation, $data);
	}
	...
	public function delete_translation_by_language($id, $language_id)
	{
		if($id && $language_id) {
			$this->database->where([$this->table_translation_key => $id, 'language_id' => $language_id]);
			$this->database->delete($this->table_translation);
		}
	}

	public function get_additional_data($table_name, $select, $where = [], $row = false) {
