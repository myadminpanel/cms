...
class Auth
{
	...
	public function __construct()
	{
		...
		$this->CI->lang->load('administrator/authentication');
	...
	public function login($identifier, $pass, $remember = false, $totp_code = null)
	{
		...
		if ($this->config_vars['ddos_protection'] && $this->config_vars['recaptcha_active'] && $this->get_login_attempts() > $this->config_vars['recaptcha_login_attempts']) {
			...
			$resp = $reCaptcha->verifyResponse($this->CI->input->server("REMOTE_ADDR"),
	...
	public function ceate_user_by_social($pass, $firstname, $lastname, $email, $type = 'normal', $image = '') {
		$valid = true;
	...
	public function create_user($email, $pass, $username = false, $firstname = '', $lastname = '', $group_id = false, $mobile = '', $image = '')
	{
		...
		if ($group_par != false) {
			...
			$this->CI->db->select('*')
				...
				->join($this->config_vars['user_to_group'],
					$this->config_vars['users'] . ".id = " . $this->config_vars['user_to_group'] . ".user_id")
