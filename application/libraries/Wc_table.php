...
class Wc_table
{
	...
	public function set_multilingual($multilingual)
	{
	...
	public function format_rows()
	{
		if ($this->rows) {
			foreach ($this->rows as $key => $row) {
						...
						foreach ($this->custom_rows as $custom_row) {
							if ($column == $custom_row['column']) {
								if (isset($custom_row['callback'])) {
									$formatted_rows[$key][$custom_row['column']] = $this->CI->Custom_model->{'callback_' . $custom_row['callback']}($data,
										$custom_row['params'],$row->id);
								}
							}
						}
					}
				...
				{
					$formatted_rows[$key]['languages'] = get_languages($row->id);
				}

				if($this->info)
				{
					$formatted_rows[$key]['info'] = get_info($row->id);
				}
				...
				if ($this->action) {
					...
					if (isset($this->action['copy']) && $this->action['copy'] === true) {
						$formatted_rows[$key]['action'] .= '<li><a href="' . site_url_multi($this->url . '/copy/') . $row->id .'?returnURL='.current_full_url().'" data-popup="tooltip" title="' . translate('copy',
								true) . '"><i class="icon-copy"></i></a></li>';
					}
					if (isset($this->action['edit']) && $this->action['edit'] === true) {
						$formatted_rows[$key]['action'] .= '<li><a href="' . site_url_multi($this->url . '/edit/') . $row->id.'?returnURL='.current_full_url().'" data-popup="tooltip" title="' . translate('edit',
	...
	public function format_heading()
	{
		...
		if($this->multilingual)
		{
			$table_heads[] = "<th style='width:auto;'><i class='icon-flag3'></i></th>";
		}
		
		if ($this->info)
		{
			$table_heads[] = "<th style='width:auto;'><i class='icon-info22'></i></th>";
		}
