...
class Tag extends Site_Controller
{
	...
	public function index($slug = false)
	{
		...
		{
			$slug = urldecode($slug);
			$tag = $this->Tag_model->filter(['name' => $slug])->one();
			if($tag)
			{
				
				$this->data['title'] = '#'.$tag->name;
				$this->breadcrumbs->push('#'.$tag->name, 'tag/'.$tag->name);
				...
				$this->data['filter']['FIND_IN_SET("'.$slug.'", tag)'] = NULL;
				...
				if($this->data['languages'])
				{
					foreach($this->data['languages'] as $key => $langauge)
					{
						...
						$this->data['language_link'][$key] = site_url($lang_slug.'tag/'.$tag->name);
