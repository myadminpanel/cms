...
class User extends Administrator_Controller
{
	...
	public function index()
	{
		...
		if (check_permission($this->controller, 'delete')) {
			$this->data['buttons'][] = [
				'type' => 'button',
		...
		// Sets Table columns
		$this->data['fields'] = ['id', 'firstname', 'lastname', 'email', 'last_activity', 'banned'];
	...
	public function trash()
	{
		...
		// Sets buttons
		if (check_permission($this->controller, 'remove')) {
			$this->data['buttons'][] = [
				'type' => 'button',
		...
		if (check_permission($this->controller, 'restore')) {
			$this->data['buttons'][] = [
				'type' => 'button',
		...
		if (check_permission($this->controller, 'clean')) {
			$this->data['buttons'][] = [
				'type' => 'a',
	...
	public function create()
	{
		...
		if ($this->input->method() == 'post') {
			if ($this->form_validation->run() == true) {
				...
				$user_id = $this->auth->create_user($general['email'], $general['password'], false,
					$general['firstname'], $general['lastname'], $general['group_id'], '');
				if ($user_id) {
					redirect(site_url_multi($this->directory . $this->controller));
				}
				else
				{
					$this->data['message'] = $this->auth->print_errors();
				}
			} else {
		...
		if ($id && ctype_digit($id)) {
			...
			if ($this->data['general']) {
				...
				if ($this->input->method() == 'post' && $this->form_validation->run() == true)
				{
					...
					$response = $this->auth->update_user($id, $general['email'], $general['password'],
						false, $general['firstname'], $general['lastname'], $general['group_id']);
					redirect(site_url_multi($this->directory . $this->controller));
				}
				else
				{
	...
	public function clean()
	{
		...
		redirect(site_url_multi($this->directory . $this->controller.'/trash'));
	}
}
