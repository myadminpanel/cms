	...
	public function index($type = false)
	{
		...
		foreach ($images as $image) {
			...
			if (is_dir($image)) {
				...
				if (isset($CKEditor)) {
					$url .= '&CKEditor=' . $CKEditor;
				}

				$CKEditorFuncNum = $this->input->get('CKEditorFuncNum');
				if (isset($CKEditorFuncNum)) {
					$url .= '&CKEditorFuncNum=' . $CKEditorFuncNum;
				}
				...
				$this->data['images'][] = array(
					...
					'name'  => $name,
					'path'  => substr($image, strlen(DIR_IMAGE)),
					'href'  =>  '/uploads/' . substr($image, strlen(DIR_IMAGE))
				);
			}
		}
		...
		if ($type == 'popup') {
			$this->template->view($this->admin_url.'/new/filemanager/popup.tpl', $this->data);
		} elseif ($type == 'ckeditor') {
			$this->template->render('filemanager/ckeditor', 'ckeditor');
		} else {
