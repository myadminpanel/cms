...
class Discount extends Api_Controller {
	...
	public function index()
	{
		...
		$rows = $this->Discount_model->filter($filter)->with_translation()->order_by('created_at', 'DESC')->limit($per_page, $page)->all();
		...
		if($rows)
		{
			foreach($rows as $row)
			{
				...
				$data->end_date = format_date($row->date, 'd F Y');
				$data->view = $row->view;
	...
	public function detail($id = false)
	{
		if($id)
		{
			$row = $this->Discount_model->filter(['status' => 1, 'id' => $id])->with_translation()->one();
			if($row)
			{
				...
				$discount->company			= $company_data;
				$discount->name				= $row->name;
				$discount->description		= strip_tags($row->description);
				$discount->image			= $this->Model_tool_image->resize($row->image, 800, 600);
				$discount->date				= format_date($row->created_at, 'd F Y');
				$discount->view				= $row->view;
				$discount->author			= get_user($row->created_by);
				$discount->author_id		= $row->created_by;
				$discount->date				= $row->date;
				$discount->comment_count	= $this->Review_model->filter(['status' => 1, 'data_id' => $row->id, 'type' => 'discount'])->count_rows();

				//Related Discount
				$related_discounts = $this->Discount->filter(['status' => 1, 'id !=' => $discount->id])->with_translation()->order_by('created_at', 'RANDOM')->limit(5, 0)->all();
				$this->data['relateds'] = [];
				if($related_discounts)
				{
					foreach($related_discounts as $row)
					{
						...
						$discount->relateds[] = $data;
					}

				}

				//Other Discount
				$other_discounts = $this->Discount->filter(['status' => 1, 'id !=' => $discount->id, 'company_id' => $row->company_id])->with_translation()->order_by('created_at', 'DESC')->limit(5, 0)->all();
				$this->data['others'] = [];
				if($other_discounts)
				{
