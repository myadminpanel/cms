...
class News extends Api_Controller {
	...
	public function index()
	{
		...
		$rows = $this->News_model->filter($filter)->with_translation()->order_by('created_at', 'DESC')->limit($per_page, $page)->all();
	...
	public function detail($id = false)
	{
		if($id)
		{
			$row = $this->News_model->filter(['status' => 1, 'id' => $id])->with_translation()->one();
			if($row)
			{
				...
				$news->id				= $row->id;
				$news->name				= $row->name;
				$news->company			= $company_data;
				$news->name				= $row->name;
				$news->description		= str_replace(["\n", "\r", "<p>", "</p>"], '', str_replace('/uploads/catalog/', 'https://citymap.az/uploads/catalog/', $row->description));
				$news->image			= $this->Model_tool_image->resize($row->image, 800, 600);
				$news->date				= format_date($row->created_at, 'd F Y');
				$news->view				= $row->view;
				$news->author			= get_user($row->created_by);
				$news->author_id		= $row->created_by;
				$news->comment_count	= $this->Review_model->filter(['status' => 1, 'data_id' => $row->id, 'type' => 'news'])->count_rows();
				...
				//Other News
				$other_news = $this->News_model->filter(['status' => 1, 'id !=' => $news->id, 'company_id' => $row->company_id])->with_translation()->order_by('created_at', 'DESC')->limit(5, 0)->all();
				$this->data['others'] = [];
				if($other_news)
				{
		...
		if($this->input->method() == 'post')
		{
			...
			if($this->input->post('translation'))
			{
				...
				if($this->form_validation->run())
				{
					...
					$discount_id = $this->News_model->insert($discount);
					...
					foreach($this->data['languages'] as $language)
					{
						if($translation[$language['id']]['name'])
						{
							...
							$this->News_model->insert_translation($discount_translation_data);
						}
					}
