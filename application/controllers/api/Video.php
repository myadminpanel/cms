...
class Video extends Api_Controller {
	...
	public function index()
	{
		...
		$rows = $this->Video_model->filter($filter)->with_translation()->order_by('created_at', 'DESC')->limit($per_page, $page)->all();
	...
	public function detail($id = false)
	{
		if($id)
		{
			$row = $this->Video_model->filter(['status' => 1, 'id' => $id])->with_translation()->one();
			if($row)
			{
				...
				$video->id				= $row->id;
				$video->name			= $row->name;
				$video->company			= $company_data;
				$video->name			= $row->name;
				$video->description		= strip_tags($row->description);
				$video->video			= $row->video;
				$video->date			= format_date($row->created_at, 'd F Y');
				$video->view			= $row->view;
				$video->author			= get_user($row->created_by);
				$video->author_id		= $row->created_by;
				$video->comment_count	= $this->Review_model->filter(['status' => 1, 'data_id' => $row->id, 'type' => 'video'])->count_rows();

				//Related Video
				$related_videos = $this->Video_model->filter(['status' => 1, 'id !=' => $video->id])->with_translation()->order_by('created_at', 'RANDOM')->limit(5, 0)->all();
				$this->data['relateds'] = [];
				if($related_videos)
				{
				...
				//Other Video
				$other_videos = $this->Video_model->filter(['status' => 1, 'id !=' => $video->id, 'company_id' => $row->company_id])->with_translation()->order_by('created_at', 'DESC')->limit(5, 0)->all();
				$this->data['others'] = [];
				if($other_videos)
				{
